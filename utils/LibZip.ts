...
function flz1_match(len, distance, op) {
  ...
  if (len < 7) {
    op.push(Buffer.from([(len << 5) + (distance >> 8), distance & 255]));
  } else {
    op.push(Buffer.from([(7 << 5) + (distance >> 8), len - 7, distance & 255]));
  }
}
...
function flz2_match(len, distance, op) {
  ...
  if (distance < MAX_L2_DISTANCE) {
    if (len < 7) {
      ...
      op.push(Buffer.from([len, distance & 255]));
    }
  } else {
    ...
    if (len < 7) {
      ...
      op.push(
        Buffer.from([(len << 5) + 31, 255, distance >> 8, distance & 255])
      );
    } else {
      distance -= MAX_L2_DISTANCE;
      op.push(Buffer.from([(7 << 5) + 31]));
      for (len -= 7; len >= 255; len -= 255) op.push(Buffer.from([255]));
      op.push(Buffer.from([len, 255, distance >> 8, distance & 255]));
    }
  }
}
...
export class LibZip {
  static flzCompress(input: Buffer) {
    return fastlz_compress(input);
  }
  static flzDecompress(input: Buffer) {
    return fastlz_decompress(input);
  }
}
